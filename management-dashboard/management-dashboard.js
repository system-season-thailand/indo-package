...
(function () {
  ...
  function openStaff(staffId) {
    ...
    var sub = '<span class="role-pill">' + ACCESS.label(s.role) + "</span>";
    var body = facts +
      '<div class="ct-2col">' +
  ...
  function openHotel(hotelId) {
    ...
    var sub = '<span class="hotel-loc">' + esc((h.city || "") + (h.area ? " · " + h.area : "")) + "</span>";
    var body = facts +
      '<div class="ct-2col">' +
  ...
  function wire() {
    ...
    el("periodControl").addEventListener("click", function (e) {
      ...
      state.period = p === "month" ? "month" : parseInt(p, 10);
      setActive("periodControl", b);
      renderOverview(); renderDaily(); renderCompanies(); renderStaff(); renderHotelsRegions();
      revealObserve();
    });

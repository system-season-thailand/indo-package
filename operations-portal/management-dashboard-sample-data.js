...
/* =====================================================================
   ▼▼▼  SAMPLE DATA SOURCE  (LAB MODE — read-only, no backend)  ▼▼▼
   ---------------------------------------------------------------------
   FUTURE SUPABASE DATA SOURCE (for Bandar):
   Replace the generated `window.MGMT_DATA` produced below with a
   READ-ONLY query from the existing Supabase quotation tables.
   Keep the EXACT same shape so no dashboard code changes:
     window.MGMT_DATA = {
       meta, destinations:[{id,name}], companies:[], staff:[],
       hotels:[{id,name,city,area,country}], quotations:[{...}]
     }
   The Management Dashboard is READ-ONLY → it never writes.
